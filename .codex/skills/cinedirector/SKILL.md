...
In **Face & Lipsync**: select the character, **Use Selected Actor**, then **Analyze Face** to confirm morph-target mapping before generating (paste unmapped morph names into the analyzer's tables if slots are missing). Provide dialogue audio (.wav directly; mp3/ogg/flac/m4a require ffmpeg) for audio-driven lipsync, or leave it empty with "Talking" checked for synthesized speech motion. Describe emotion in plain language (`scared`, `angry`, ... with `slightly`/`very` and `then` arcs). Tune with the strength sliders: **Mouth** (travel), **Articulation** (crispness — below 1 mumbly, above 1 snappy full enunciation; also scales the automatic lip pre-shaping/jaw co-articulation/teeth-reveal output since those derive from the final mouth channels), **Emotion** (brow/full-face pose strength; mouth-area emotion automatically yields while words are spoken). The bake is a curves-only additive UAnimSequence layered onto the character in the open Level Sequence — body animation is untouched; audio lands on the sequence's audio track.
